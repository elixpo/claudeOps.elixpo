...
if ask "Remove God Mode agents from $AGENTS_DIR?"; then
  count=0
...
if ask "Remove God Mode CLAUDE.md?"; then
...
if ask "Remove God Mode bin scripts from $CLAUDE_DIR/bin/?"; then
  count=0
...
if ask "Remove God Mode hook scripts from $CLAUDE_DIR/hooks/?"; then
  count=0
...
if ask "Remove God Mode hooks from settings.json? (will preserve non-God-Mode hooks)"; then
  python3 -c "
import json, os
...
# God Mode hook signatures to identify and remove
god_mode_signatures = [
...
print(f'  [+] Removed {removed} God Mode hook entries from settings.json')
...
if ask "Remove God Mode MCP servers from ~/.claude.json?"; then
  python3 -c "
import json, os
